...
| H-08 | Approval/closure/merge safety variants | Stale or later-invalidated approval, Gate `HUMAN`, non-allowlisted closure, failed checks/mergeability, and reserved matters cannot merge; exact allowlisted closure from approved HEAD preserves authorization; auto-merge stays disabled | PASS | `test_h08_stale_approval_does_not_close_new_head` and `test_h08_*` safety tests |
