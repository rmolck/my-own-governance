...
The finalizer may derive one closure commit from exactly the approved HEAD without renewed review only when its complete diff is allowlisted to `docs/WORK_QUEUE.md`, records only the approved checkpoint's closure metadata (including `AI_REVIEW -> DONE`), and changes no code, tests, requirements, normative decisions, or substantive behavior. Any other path or content invalidates delegated authorization and requires review of the new HEAD. Closure and merge target exactly that derived HEAD and form one operational finalization—not another checkpoint, semantic decision, or principal transition.
