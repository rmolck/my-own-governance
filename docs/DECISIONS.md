...
**Consequences:** Later substantive changes or a later `AI_REWORK` or `HUMAN_REQUIRED` invalidate approval. A closure commit may preserve derived authorization without renewed review only when it starts exactly at the approved HEAD and its entire diff is allowlisted to `docs/WORK_QUEUE.md` closure metadata; code, tests, requirements, normative decisions, or any other file invalidate it. The finalizer merges exactly the verified HEAD using movement protection when available, after verifying PR identity and legitimacy, mergeability, required/configured checks, gate and human-reserved boundaries, and non-destructive operation. A failed operational precondition does not automatically mean `BLOCKED`. Approval never enables generic GitHub auto-merge.
