...
For `AI_REWORK`, address findings on the same branch/PR and resubmit. For `HUMAN_REQUIRED`, record the material choice and stop at the gate. For `BLOCKED`, record the objective dependency and revalidation evidence. Approval permits queue closure to `DONE`, not merge. The HUMAN OWNER authorizes merge.
