...
For Gate `AI`, durable `AI_SUPERVISOR: APPROVED` on the relevant HEAD is semantic completion and conditional merge authorization. A finalizer may perform the strictly allowlisted closure and merge as one operational finalization; it does not perform a second semantic transition or select more work. It must verify every safeguard in `AUTONOMY.md`. A failed mergeability or checks precondition pauses execution without automatically recording `BLOCKED`, and approval never implicitly enables auto-merge.
